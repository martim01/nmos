...
ReceiverNode::ReceiverNode(const std::string& sLabel, const std::string& sDescription, enumTransport eTransport, const std::string& sDeviceId, enumType eFormat, int flagsTransport) :
Receiver(sLabel, sDescription, eTransport, sDeviceId, eFormat, flagsTransport)
{

}

ReceiverNode::ReceiverNode() : Receiver()
{
}


void ReceiverNode::CommitActivation()
{
...
bool ReceiverNode::Stage(const connectionReceiver& conRequest)
{
    bool bOk(true);
    m_mutex.lock();
...
void ReceiverNode::Activate(bool bImmediate)
{
    m_bActivateAllowed = false;
