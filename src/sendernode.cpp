...
void SenderNode::Activate(bool bImmediate)
{
...
void SenderNode::CommitActivation()
{
...
bool SenderNode::Commit(const ApiVersion& version)
{
...
void SenderNode::SetupActivation(const std::string& sSourceIp, const std::string& sDestinationIp, const std::string& sSDP)
{
...
void SenderNode::SetDestinationDetails(const std::string& sDestinationIp, unsigned short nDestinationPort)
{
...
void SenderNode::MasterEnable(bool bEnable)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
bool SenderNode::Stage(const connectionSender& conRequest, std::shared_ptr<EventPoster> pPoster)
{
    bool bOk(true);
