...
{
    pml::log::log(pml::log::Level::kInfo, "pml::nmos") << "Request completed" ;
    ...
    {
        pml::log::log(pml::log::Level::kError, "pml::nmos") << "Request completed: Failed" ;
    }
}
...
int RegistryServer::AnswerToConnection(void *cls, MHD_Connection* pConnection, const char * url, const char * method, const char * sVersion, const char * upload_data, size_t * upload_data_size, void **ptr)
{
    ...
    pml::log::log(pml::log::Level::kDebug, "pml::nmos") << "AnswerToConnection: " << sAddr ;

    string sMethod(method);
    if (NULL == *ptr)
    {
        pml::log::log(pml::log::Level::kDebug, "pml::nmos") << "Initial connection" ;
        ...
        if("POST" == sMethod)
        {
            pml::log::log(pml::log::Level::kDebug, "pml::nmos") << "Initial connection: " << sMethod ;
            if("POST" == sMethod)
            {
        ...
        {
            pml::log::log(pml::log::Level::kDebug, "pml::nmos") << "Initial connection: GET" ;
        }
        *ptr = (void *) pInfo;

        pml::log::log(pml::log::Level::kDebug, "pml::nmos") << "Initial connection: return MHD_YES" ;
        return MHD_YES;
    }

    pml::log::log(pml::log::Level::kDebug, "pml::nmos") << "RegistryServer: " << url ;
    if("GET" == string(sMethod))
    {
        RegistryInfo* pInfo = reinterpret_cast<RegistryInfo*>(*ptr);
        pml::log::log(pml::log::Level::kDebug, "pml::nmos") << "Actual connection: GET" ;
        return DoHttpGet(pConnection, url, pInfo);
    }
    else if("DELETE" == string(sMethod))
    {
        RegistryInfo* pInfo = reinterpret_cast<RegistryInfo*>(*ptr);
        pml::log::log(pml::log::Level::kDebug, "pml::nmos") << "Actual connection: DELETE" ;
        return DoHttpDelete(pConnection, url, pInfo);
    }
    else if("POST" == string(sMethod))
    {
        pml::log::log(pml::log::Level::kDebug, "pml::nmos") << "Actual connection: POST" ;
        RegistryInfo* pInfo = reinterpret_cast<RegistryInfo*>(*ptr);
...
, nPort, NULL, NULL, &RegistryServer::AnswerToConnection, this, MHD_OPTION_NOTIFY_COMPLETED, RequestCompleted, NULL, MHD_OPTION_THREAD_POOL_SIZE,4, MHD_OPTION_END);
    if(m_pmhd)
    {
        pml::log::log(pml::log::Level::kInfo, "pml::nmos") << "RegistryServer: " << nPort << " Init: OK" ;
    }
    else
    {
        pml::log::log(pml::log::Level::kInfo, "pml::nmos") << "RegistryServer: " << nPort << " Init: Failed" ;
    }
...
{
    pml::log::log(pml::log::Level::kInfo, "pml::nmos") << "RegistyServer: " << sPath << " " << sJson ;
    ...
    pml::log::log(pml::log::Level::kInfo, "pml::nmos") << "Response: " << sResponse << " Code:" << nCode ;
    return nCode;
}
...
int RegistryServer::PostJsonNmosResource(const std::string& sJson, std::string& sReturn, std::string& sLocation)
{
    ...
    pml::log::log(pml::log::Level::kInfo, "pml::nmos") << "RESOURCE TYPE: " << jsRequest["type"].asString() ;
