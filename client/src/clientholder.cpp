...
template<class T> std::shared_ptr<T> ClientHolder<T>::UpdateResource(const Json::Value& jsData)
{
    ...
    if(pResource)
    {
        ...
        pml::log::log(pml::log::Level::kInfo, "pml::nmos") << "NMOS: " << jsData["id"].asString() << " found already " ;
        if(pResource->UpdateFromJson(jsData))
        {
            pml::log::log(pml::log::Level::kInfo, "pml::nmos") <<  "NMOS: " << jsData["id"].asString() << " updated " ;
        }
        else
        {
            pml::log::log(pml::log::Level::kInfo, "pml::nmos") << "NMOS: " << "Found node but json data incorrect: " << pResource->GetJsonParseError() ;
        }
    }
    return pResource;
}
